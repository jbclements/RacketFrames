...
(provide:
 ...
 [frame-cseries-tabulate (DataFrame Symbol -> DataFrame)])
...
(define (frame-cseries-tabulate frame symbol)
  (let ((tab (tabulate (data-frame-cseries frame symbol))))
    (new-data-frame (list (cons symbol (new-CSeries (Tabulation-nominals tab)))
